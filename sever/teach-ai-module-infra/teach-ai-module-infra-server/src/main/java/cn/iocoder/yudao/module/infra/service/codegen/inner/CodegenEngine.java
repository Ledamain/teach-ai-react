package cn.iocoder.teach-ai.module.infra.service.codegen.inner;
...
import cn.iocoder.teach-ai.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.iocoder.teach-ai.framework.common.util.date.LocalDateTimeUtils;
import cn.iocoder.teach-ai.framework.common.util.object.BeanUtils;
import cn.iocoder.teach-ai.framework.common.util.object.ObjectUtils;
import cn.iocoder.teach-ai.framework.common.util.string.StrUtils;
...
import cn.iocoder.teach-ai.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.iocoder.teach-ai.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.iocoder.teach-ai.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.iocoder.teach-ai.module.infra.enums.codegen.CodegenSceneEnum;
import cn.iocoder.teach-ai.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.iocoder.teach-ai.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.iocoder.teach-ai.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 teach-ai-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 teach-ai-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.iocoder.teach-ai.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/teach-ai-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/teach-ai-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "teach-ai-module-${table.moduleName}/" + // 顶级模块
                "teach-ai-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "teach-ai-module-${table.moduleName}/" + // 顶级模块
                "teach-ai-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "teach-ai-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "teach-ai-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "teach-ai-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "teach-ai-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
